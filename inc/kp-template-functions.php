...
function kapee_setup_loop( $args = array() ) {
	...
	$default_args = array(
		...
		'products-columns'          			=> kapee_get_option( 'products-columns', 4 ),
		'products-element'          			=> '',
		'products-columns-mobile'          		=> kapee_get_option( 'products-columns-mobile', 2 ),
...
if( !function_exists( 'kapee_blog_wrapper_classes' ) ):
	function kapee_blog_wrapper_classes() {			
		...
		if( kapee_get_loop_prop('name') == 'related-posts'){
			...
			if(kapee_get_option('related-posts-display', 'slider') =='slider') {
			...
			if($blog_post_style == 'blog-grid' && kapee_get_loop_prop('name') != 'posts-slider-shortcode'){
				$classes[] ='row';
			}
...
if ( ! function_exists( 'kapee_template_single_related_portfolio' ) ) {
	...
	function kapee_template_single_related_portfolio( $args =array() ) {
		...
		$post_id = get_the_id();
		$taxonomy = kapee_get_option('related-portfolios-taxonomy', 'post_tag');
...
if ( ! function_exists( 'kapee_newsletter_popup' ) ) {	
	...
	function kapee_newsletter_popup(){
		...
		$showpopup_on 		= kapee_get_option( 'newsletter-popup-on', 'all-pages' );	
		if( $showpopup_on == 'front-page' && !is_front_page() ){
			return;
		}
...
if ( ! function_exists( 'kapee_mobile_bottom_navbar' ) ) {	
	...
	function kapee_mobile_bottom_navbar(){
		...
		foreach ( $mobile_elemets['enabled'] as $element => $value ) {
			...
			$args['elements'][$element] = apply_filters( 'kapee_mobile_bottom_navbar_element'.$element, $element_args );
